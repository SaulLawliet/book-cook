...
class BookCook(object):
    ...
    def cook(self, url):
        for ie in self._ies:
            if not ie.suitable(url):
                continue
            logger.info("use extractor: " + ie.__name__)
    ...
    def _process_content(self, content):
        ...
        for img in bs.select("img"):
            ...
            if id not in self._id_hash:
                ...
                self.book.add_item(
                    epub.EpubImage(
                        uid=id, file_name=id, media_type=media_type, content=body
                    )
                )
    ...
    def _epub_create_html(self, title, file_name, content):
        rtn = epub.EpubHtml(
            title=title, file_name=file_name, content=self._process_content(content)
        )
    ...
    def _cook(self, url, ie):
        ...
        if "cover" in ie_result:
            ...
            self.book.set_cover(
                cover_name, utils.http_get_content(ie_result["cover"], allow_cache=True)
            )
        ...
        logger.info(json.dumps(ie_result, indent=2, ensure_ascii=False))
